# Roadmap Paperasse SaaS

Date : 2026-05-19  
Sources : `Backend/cadrage-architecture-backend-paperasse-v3.md`, `Backend/annexe cadrage architecture-deterministe-vs-ia-paperasse.md`, `Backend/analyse-execution-etapes-1-4.md`

## Lecture rapide

Paperasse SaaS transforme le repo `romainsimon/paperasse`, aujourd'hui plutôt CLI/filesystem, en application web SaaS.

L'idée centrale est simple :

1. Construire l'expérience SaaS autour de Paperasse : comptes utilisateurs, entreprises, imports, écrans, base de données, documents téléchargeables.
2. Réutiliser au maximum les scripts, templates, données et skills du repo Paperasse existant.
...
- **Implémenté localement** : disponible dans Paperasse3, couvert par tests et validations locales.
- **Préparé ou réutilisable** : présent dans `vendor/paperasse` ou dans l'architecture, mais pas encore branché comme fonctionnalité SaaS complète.
...
- Paperasse3 couvre fortement la boucle locale `import → catégorisation → écritures → documents → contrôle → clôture`.
- Le produit est déjà très avancé sur l'auditabilité interne : preuves, journal, génération Paperasse, contrôle pré-clôture, OD, archive et verrouillage.
...
- Monde Paperasse : fichiers `company.json`, `data/journal-entries.json`, scripts Node.js, templates, données réglementaires.

Ce pont est actuellement matérialisé par `PaperasseRuntime`. Dans le cadrage backend, il est appelé `PaperasseAdapter`.

Son rôle :

- convertir une entreprise SaaS en `company.json` compatible Paperasse ;
- convertir les écritures comptables en `data/journal-entries.json` ;
- préparer un dossier temporaire compatible avec les scripts Paperasse ;
...
Un utilisateur doit pouvoir configurer son entreprise, importer des transactions, obtenir des catégorisations, corriger les cas incertains, générer des écritures équilibrées et produire les premiers documents Paperasse.
...
### Ce qui est réutilisé depuis Paperasse
...
- Le repo Paperasse dans `vendor/paperasse`.
...
- `PaperasseRuntime.toCompanyJson()` compatible avec `company.example.json`.
- Build Remix.
...
- Ne pas considérer les connecteurs Qonto/Stripe comme couverts tant que les scripts `vendor/paperasse/integrations/*/fetch.js` ne sont pas branchés dans un vrai module SaaS d'intégrations.
...
- CI/CD.
- Fork ou submodule Paperasse décidé et documenté.
- Adapter Paperasse stabilisé.
- Gestion des erreurs structurée.
- Journal d'activité technique minimal.

### Points techniques

- Le repo Paperasse reste une dépendance locale/forkée.
- Les scripts Paperasse doivent être audités :
...
Objectif : permettre l'import automatique depuis les connecteurs fournis par Paperasse.
...
Statut local : implémentée comme base production-shaped avec journal filtrable/exportable, documents enrichis et exécution Paperasse structurée.
...
- `PaperasseExecutionCenter` : scripts Paperasse avec args, timeout, stdout/stderr, exitCode, scriptVersion et message utilisateur.
- Génération FEC.
- Génération balance.
- Génération bilan.
...
Statut local : implémentée comme gel d'auditabilité avec preuves journal, génération Paperasse et paquet de preuve local.
...
- États financiers et exports : scripts Paperasse.
...
- Les scripts Qonto/Stripe existent dans `vendor/paperasse`, mais les connecteurs SaaS ne sont pas encore livrés.
...
- Brancher les templates Paperasse existants pour la liasse 2033/2065.
...
- `PaperasseExecutionCenter`
- `PaperassePdfRenderer`
- `DocumentEvidenceBundle`
- `AnnualClosingCenter`

Décisions verrouillées pour la Phase 8.5 :

- Les scripts Paperasse restent appelés en CLI via workdir et `execFile`.
...
- Exercice actif via cookie/query `paperasse_fiscal_year_id`.
...
Décision : Paperasse intégrera l'Open Banking via un **provider agréé** ou opérant comme AISP, par exemple Bridge, Powens, GoCardless Bank Account Data, Tink, Yapily ou équivalent. Paperasse ne fera pas d'intégration PSD2 directe banque par banque avant maturité production.
...
- `paperasse-web` : app Remix/API.
- `paperasse-worker` : jobs BullMQ.
- PostgreSQL managé.
- Redis managé.
...
- Durée des scripts Paperasse.
- Erreurs des scripts Paperasse.
...
- `paperasse_lookup_hits_total`.
- `paperasse_lookup_misses_total`.
- `paperasse_lookup_hit_rate`.
- `paperasse_ai_calls_saved_total`.
- `paperasse_ai_cost_saved_cents`.
- `paperasse_vendor_mappings_total`.
...
Objectif : stabiliser le socle Phase 16 avant les extensions post-beta, avec readiness beta visible dans Paperasse.
...
Objectif : exploiter les autres capacités du repo Paperasse.
...
- Domaine notaire.
- Domaine syndic.
- Autres skills Paperasse spécialisés.
...
1. Garder Paperasse en submodule Git ou en clone/fork copié dans `vendor/paperasse`.
...
Décision : pour la Phase 8.5, les scripts Paperasse restent **CLI-only**.

Conséquences :

- Le SaaS prépare un workdir compatible Paperasse.
- Les scripts sont appelés avec `execFile`.
- Les détails d'exécution restent confinés dans `PaperasseExecutionCenter` et les renderers dédiés.
...
Décision : l'accès bancaire live passera d'abord par un **provider Open Banking**. Paperasse ne s'enregistrera pas comme AISP et n'implémentera pas les API DSP2 banque par banque avant la beta.
