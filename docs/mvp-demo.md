# Démo MVP locale Paperasse

## Pré-requis

- PostgreSQL local avec `DATABASE_URL` pointant vers une base locale dont le nom contient `paperasse`.
...
En mode `AUTH_MODE=dev`, la page `/demo` permet de charger ces mêmes datasets directement dans Paperasse. Elle demande une confirmation car le reset est destructif, puis redirige vers le dashboard.
...
- Les erreurs Paperasse doivent apparaître dans la page Documents avec un message lisible, sans page `Application Error`.
...
- Le FEC officiel est le fichier `.txt` généré par Paperasse depuis `Documents`. C'est lui qui porte le format fiscal attendu.
