...
Before implementing chat and billing, Paperasse SaaS must cover the three accounting gaps identified by the MVP coverage audit: TVA-aware JournalEntries, a structured fiscal package draft, and external expert-comptable review.
...
The fiscal package is source-first. `TaxPackageTemplateRenderer` produces the structured HTML/Markdown source with case labels, amounts, and calculation references. That source is the business truth. `PaperassePdfRenderer` may derive a PDF via Paperasse CLI and Puppeteer, but PDF failure does not block the beta coverage if the structured source exists.

Paperasse scripts stay CLI-only for this phase. The SaaS prepares a workdir and calls scripts via `execFile`; no upstream refactor to importable generators is required before beta.
...
- TVA logic is testable without importing Remix routes or Paperasse scripts.
