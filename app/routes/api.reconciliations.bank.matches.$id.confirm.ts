...
export async function action(args: ActionFunctionArgs) {
  ...
  try {
    ...
    const match = await new BankLineReconciliationCenter().confirmMatch(workspace, { matchId: String(args.params.id), note: String(form.get("note") || "") || null });
    await new ActivityLogCenter().recordActivity(workspace, { action: "reconciliation.bank_match_confirmed", entityType: "reconciliation_match", entityId: match.id, metadata: { status: match.status } });
