...
export async function action(args: ActionFunctionArgs) {
  ...
  try {
    ...
    await new AttachmentLinkCenter().unlinkAttachment(workspace, String(args.params.id));
    if (args.request.headers.get("accept")?.includes("application/json")) return json({ ok: true });
    return redirect(`${returnTo}?success=${encodeURIComponent("Rattachement retiré")}`);
  } catch (error) {
