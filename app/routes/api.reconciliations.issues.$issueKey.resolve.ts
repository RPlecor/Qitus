...
export async function action(args: ActionFunctionArgs) {
  ...
  try {
    ...
    const issue = await new ReconciliationReviewWorkflow().resolveIssue(workspace, { issueKey: String(args.params.issueKey), note: String(form.get("note") || "") });
    await new ActivityLogCenter().recordActivity(workspace, { action: "reconciliation.issue_resolved", entityType: "reconciliation_issue", entityId: issue.issueKey, metadata: { code: issue.code } });
