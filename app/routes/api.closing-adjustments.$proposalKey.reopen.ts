...
export async function action(args: ActionFunctionArgs) {
  ...
  try {
    ...
    const proposal = await new ClosingAdjustmentReviewWorkflow().reopenRejected(workspace, {
      proposalKey: String(params.proposalKey),
      note: String(form.get("note") || ""),
    });
    if (request.headers.get("accept")?.includes("application/json")) return json({ proposal });
    return redirect(`/controle/od/${encodeURIComponent(proposal.proposalKey)}?success=${encodeURIComponent("OD réouverte")}`);
  } catch (error) {
