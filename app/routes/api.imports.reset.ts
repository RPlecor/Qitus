...
export async function action(args: ActionFunctionArgs) {
  ...
  try {
    ...
    const result = await new ImportCleanupCenter().resetFiscalYearImports(workspace, {
      confirmation: await readConfirmation(request),
    });
    if (request.headers.get("accept")?.includes("application/json")) return json({ result });
    return redirect(`/imports?reset=${encodeURIComponent(`${result.importCount} import(s) supprimé(s)`)}`);
  } catch (error) {
