...
export async function action(args: ActionFunctionArgs) {
  ...
  try {
    const item = await new ExpertReviewWorkflow().createReviewItem({ kind: "shared", token: String(args.params.token) }, {
    ...
    if (args.request.headers.get("accept")?.includes("application/json")) return json({ item });
