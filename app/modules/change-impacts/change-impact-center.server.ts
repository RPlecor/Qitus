...
export class AutomationImpactSource implements ChangeImpactSource {
  ...
  async listImpacts(workspace: CompanyWorkspace): Promise<ChangeImpact[]> {
    ...
    return [impact({
      ...
      severity: "warning",
      title: summary.validationRequired > 0 ? "Validations requises" : "Dossier à mettre à jour",
      message: `${summary.safeRunnable} traitement(s) sûr(s), ${summary.suggestions} validation(s) conseillée(s), ${summary.validationRequired} brouillon(s) ou action(s) à relire.`,
