...
export class CategorizationEngine {
  ...
  async categorize(transactions: CategorizationTransaction[], context: CategorizationContext): Promise<CategorizationSuggestion[]> {
    ...
    for (const transaction of transactions) {
      const correction = matchCorrection(transaction, context.correctionRules);
      if (correction) {
        resolved.set(transaction.id, correction);
        continue;
      }

      const vendor = matchVendor(transaction, context.vendorMappings);
      if (vendor) {
  ...
  private async categorizeResidualWithAi(transactions: CategorizationTransaction[], context: CategorizationContext): Promise<CategorizationSuggestion[]> {
    ...
    try {
      ...
      return transactions.map((transaction) => byTransaction.get(transaction.id) ?? buildReviewSuggestion(transaction, "IA response omitted this transaction."));
    } catch (error) {
      const reason = error instanceof Error ? error.message : "IA provider failed.";
      return transactions.map((transaction) => buildReviewSuggestion(transaction, reason));
    }
  }
}

function matchCorrection(transaction: CategorizationTransaction, rules: CorrectionRuleInput[]): CategorizationSuggestion | null {
  ...
  return buildSuggestion(transaction, {
    accountDebit: transaction.type === "CREDIT" ? "5121" : rule.preferredAccount,
    accountDebitLabel: transaction.type === "CREDIT" ? "Banque" : rule.preferredAccountLabel,
    accountCredit: transaction.type === "CREDIT" ? rule.preferredAccount : "5121",
    accountCreditLabel: transaction.type === "CREDIT" ? rule.preferredAccountLabel : "Banque",
...
function matchVendor(transaction: CategorizationTransaction, mappings: VendorMappingInput[]): CategorizationSuggestion | null {
  const counterparty = transaction.counterparty?.toLowerCase() ?? "";
  ...
  for (const mapping of mappings) {
    ...
    return buildSuggestion(transaction, {
      accountDebit: transaction.type === "CREDIT" ? "5121" : mapping.accountDebit,
      accountDebitLabel: transaction.type === "CREDIT" ? "Banque" : mapping.accountLabel,
      accountCredit: transaction.type === "CREDIT" ? mapping.accountDebit : mapping.accountCredit ?? "5121",
      accountCreditLabel: transaction.type === "CREDIT" ? mapping.accountLabel : "Banque",
...
function buildReviewSuggestion(transaction: CategorizationTransaction, reason: string): CategorizationSuggestion {
  return {
    transactionId: transaction.id,
    accountDebit: "471",
