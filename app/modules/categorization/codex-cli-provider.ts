  ...
  async categorize(transactions: CategorizationTransaction[], context: CategorizationContext): Promise<CategorizationSuggestion[]> {
    ...
    try {
      ...
      return (parsed.categorizations ?? []).map((suggestion) => ({
        ...
        accountCredit: suggestion.accountCredit ?? context.accountRoles.bank.account,
...
function buildPrompt(transactions: CategorizationTransaction[], context: CategorizationContext) {
  return [
    ...
    `Compte banque de reference: ${context.accountRoles.bank.account} (${context.accountRoles.bank.label}).`,
    `Compte d'attente de reference: ${context.accountRoles.suspense.account} (${context.accountRoles.suspense.label}).`,
    "Pour les debits bancaires: debit = compte de charge/attente, credit = compte banque de reference.",
    "Pour les credits bancaires: debit = compte banque de reference, credit = compte de produit/attente.",
    "Si le libelle est opaque, insuffisant ou ambigu: confidence LOW, compte d'attente de reference, rationale courte.",
