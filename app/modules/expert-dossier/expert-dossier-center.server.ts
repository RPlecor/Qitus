...
export class ExpertDossierCenter {
  ...
  async listDossierSections(workspace: CompanyWorkspace): Promise<ExpertDossierSection[]> {
    ...
    return [
      ...
      section("tax_package", "Liasse fiscale", taxPackage.status === "ready" ? "ready" : taxPackage.status === "warning" ? "partial" : "blocked", taxPackage.status === "blocked" ? "high" : taxPackage.status === "warning" ? "medium" : "low", taxPackage.label, taxPackage.sourceFilename ? [taxPackage.sourceFilename] : [], [...taxPackage.missingSections, ...taxPackage.warnings], "/documents"),
      ...
      section("evidence", "Justificatifs", evidence.requiredMissing > 0 ? "blocked" : evidence.missing > 0 ? "partial" : "ready", evidence.requiredMissing > 0 ? "high" : evidence.missing > 0 ? "medium" : "low", `${evidence.satisfied}/${evidence.total} preuve(s) satisfaite(s)`, [`${evidence.satisfied} preuve(s)`], evidence.missing > 0 ? [`${evidence.missing} preuve(s) manquante(s)`] : [], "/couverture/evidence"),
